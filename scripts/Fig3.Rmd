...
# ---- insurance ----
insure.list <- readRDS(here('data','HAB_sim_out',paste0('Insurance_x_HABhi_',nmat/1000,'k.rds'))); names(insure.list)

insure.sim <- insure.list[[1]]
insure.one.sim <- insure.list[[2]]
insure.wait.sim <- insure.list[[3]]
insure.no.sim <- insure.list[[4]]

# ---- disaster relief 1 ----
dr1list <- readRDS(here('data','HAB_sim_out',paste0('DisasterRelief1_x_HABhi_',nmat/1000,'k.rds'))); names(dr1list)

dr1.sim <- dr1list[[1]]
dr1.one.sim <- dr1list[[2]]
dr1.wait.sim <- dr1list[[3]]
dr1.no.sim <- dr1list[[4]]


# ---- disaster relief 2 ----
dr2list <- readRDS(here('data','HAB_sim_out',paste0('DisasterRelief2_x_HABhi_',nmat/1000,'k.rds'))); names(dr2list)

dr2.sim <- dr2list[[1]]
dr2.one.sim <- dr2list[[2]]
dr2.wait.sim <- dr2list[[3]]
dr2.no.sim <- dr2list[[4]]


# ---- supplemental diversification ----
sdiv.list <- readRDS(here('data','HAB_sim_out',paste0('SupplementaryDiv_x_HABhi_',nmat/1000,'k.rds'))); names(sdiv.list)

sdiv.sim <- sdiv.list[[1]]
sdiv.one.sim <- sdiv.list[[2]]
...
png(here('doc','drafts','supplement','FigS8-inflink.png'),res=200,height=1830,width=1400)
plot_grid(plotlist=list(p1,p2,p3),nrow=3,rel_heights=c(1.08,0.425,0.9),rel_widths=c(1,0.05),
...
**final plot: three panels, panel 1 with legend**
```{r}
...
png(here('doc','drafts','Fig3-change.png'),res=200,height=1820,width=1400)
plot_grid(plotlist=list(p1,p2,p3),nrow=3,rel_heights=c(0.99,0.44,0.90),rel_widths=c(1,0.05),
...
**for press**  (max width 2000, res 300dpi, jpeg file)
```{r}

table.palette.legend <- c("#04BC09", "#ABF1AD","#a6611a","#dfc27d","grey85")


p1 <- plot3a[[3]] + labs(subtitle='(a) Waiting for crab') +
  # write over legend to change order of colors
  scale_fill_discrete(name="Response",type=table.palette.legend, drop=FALSE, na.translate=FALSE, 
                      limits=c("Positive-Strong","Positive-Weak","Negative-Strong","Negative-Weak","Equivocal")) +  
  geom_vline(aes(xintercept=9.5), linewidth=0.5) +
  geom_vline(aes(xintercept=12.5), linewidth=0.5)+ guides(
    fill = guide_legend(order = 1,ncol=3,title.vjust=0.9, vjust=-5),
    shape = guide_legend(order = 0)) +
  theme(plot.subtitle=element_text(size=12,vjust=-65,hjust=-1.05),
        legend.position="top",legend.title=element_text(size=9.5, face='bold'),legend.text=element_text(size=8),
        legend.margin=margin(t=0, r=0, b=-1.5, l=0, unit="cm"), legend.byrow = FALSE,
        axis.text.y=element_text(size=9.5),
        plot.margin=margin(unit(c(0, 60.5, 0, 0), "points")))

p2 <- plot3b[[3]] + labs(subtitle='(b) No effect') +
 geom_vline(aes(xintercept=9.5), linewidth=0.5) +
  geom_vline(aes(xintercept=12.5), linewidth=0.5) + 
  theme(plot.subtitle=element_text(size=12,vjust=-10,hjust=-0.85),
        legend.position="none", axis.text.x=element_blank(),
        axis.text.y=element_text(size=9.5),
        plot.margin=margin(unit(c(-15, 60.5, 0, 0.5), "points")))

p3 <- plot3c[[3]]  + labs(subtitle='(c) Switching targets') +
  geom_vline(aes(xintercept=9.5), linewidth=0.5) +
  geom_vline(aes(xintercept=12.5), linewidth=0.5) +
  scale_x_discrete(position = "bottom",expand = c(0.05, 0.05)) +
  theme(plot.subtitle=element_text(size=12,vjust=-2,hjust=-1.09),
        legend.position="none",axis.text.x=element_text(angle=315,hjust=0),
        axis.text.y=element_text(size=9.5),
        plot.margin=margin(unit(c(0, 60.5, 0, 0), "points")))

jpeg(here('doc','drafts','for_press','Fig3-change.jpeg'),res=300,width=2000,height=2750)
plot_grid(plotlist=list(p1,p2,p3),nrow=3,rel_heights=c(1,0.4,0.93),rel_widths=c(1,0.05))
dev.off()
