...
ldiv.sim <- ldiv.list[[1]]

# ---- invested livelihood diversification ----
ldiv2.sim <- ldiv.list[[2]]
```
...
### Fig S5a: coping --> adaptive maintenance

```{r}
tmplist <- list(sq.sim,insure.sim,
                dr1.sim,dr2.sim,
                fdiv1.sim,fdiv2.sim,
                sdivS.sim)
...
hab.strat.outA <- compare.models.impact.tbl(sim.list=tmplist,
                         ...
                         common.variables=FALSE,variable.key=fig1_vars,variable.order="specific_order",
...
hab.strat.plotA <- hab.strat.outA[[3]]+
  ...
  scale_fill_discrete(name="Response",type=table.palette.legend, drop=FALSE, na.translate=FALSE,  limits=c("Positive-Strong","Positive-Weak","Negative-Strong","Negative-Weak","Equivocal")) + 
...
### Fig S5b: transformative
 
```{r}
tmplist2 <- list(ldiv1.sim,ldiv2.sim)
...
hab.ldiv.out <- compare.models.impact.tbl(sim.list=tmplist2,
                         ...
                         common.variables=FALSE,variable.key=fig1_vars,variable.order="specific_order", main.axis="Y",
                         table.palette = table.palette.orange)
...
```{r  echo=TRUE, eval=FALSE}
png(here('doc','drafts','supplement','FigS5.png'),res=300,width=2200,height=2400)
...
## Change from SQ (Fig 2)

### Fig 2a: coping --> adaptive maintenance
...
tmplist2 <- list(ldiv1.sim,ldiv2.sim)
names(tmplist2) <- c("Imposed New Livelihoods", "Invested New Livelihoods")
hab.ldiv.out <- compare.baseline.impact.tbl(sim.list=tmplist2,
                                          perturb.list=list(c(`HAB`=1),
                                                            c(`HAB`=1,`Transition support`=1)),
                                          monitor.list=list(NA,NA),
                         strong_lower=0.8, weak_lower=0.6, epsilon=1.0E-5,
...
png(here('doc','drafts','Fig2-change.png'),res=300,width=2200,height=2300)
plot_grid(plotlist=list(hab.strat.plotA,hab.ldiv.plot),nrow=2, rel_heights=c(1,0.55), labels=c("(a)","(b)"))
dev.off()
...
top <- hab.strat.outA[[3]]+
  ...
  scale_fill_discrete(name="Response",type=table.palette.legend, drop=FALSE, na.translate=FALSE, 
                      limits=c("Positive-Strong","Positive-Weak","Negative-Strong","Negative-Weak","Equivocal")) + 
  guides(fill = guide_legend(order = 1, nrow=2), shape = guide_legend(order = 0)) +
  theme_bw() + theme(plot.subtitle=element_text(size=12,vjust=-50,hjust=-0.61),
                     ...
                     legend.spacing.x=unit(0.008,"cm"),
                     plot.margin=margin(unit(c(-15, 50.5, 1.5, -5), "points")))
  
## transform
lower <- hab.ldiv.out[[3]] +
  geom_vline(aes(xintercept=10.5), linewidth=0.5) +
  geom_vline(aes(xintercept=13.5), linewidth=0.5) +
  labs(subtitle='(b)') +
  scale_x_discrete(position = "bottom",expand = c(0.05, 0.05)) +
  theme(plot.subtitle=element_text(size=12,vjust=-1,hjust=-0.61),
        legend.position = "none",
        axis.text.x=element_text(angle=315,hjust=0),
                     axis.text.y=element_text(size=9.5),
        panel.grid.major.x=element_line(linewidth=0.5,linetype=3,color='grey30'),
        plot.margin=margin(unit(c(-10, 50.5, -10, 27), "points"))) 

## save
jpeg(here('doc','drafts','for_press','Fig2-change.jpeg'),res=300,width=1970,height=2150)
plot_grid(plotlist=list(top,lower),nrow=2, rel_heights=c(1,0.55))
dev.off()
