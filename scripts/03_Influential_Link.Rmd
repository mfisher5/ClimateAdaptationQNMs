...
save
```{r}
out <- list(dr1.sim, dr1.one.HABhi.sim,dr1.wait.sim,dr1.no.sim)
names(out) <- c("Default","Choose one","Wait for crab","No effect")
saveRDS(out,here('data','HAB_sim_out','DisasterRelief1_x_HABhi_50k_rerun.rds'))
```
...
save
```{r}
out <- list(dr2.sim,dr2.one.sim, dr2.wait.sim,dr2.no.sim)
names(out) <- c("Default","Choose one","Wait for crab","No effect")
...
tmp_list <- list(sq.sim, sq.one.sim, insure.sim,insure.one.sim,
                 ...
                 sdivS.sim,sdivS.one.sim); names(tmp_list) <- c("Status Quo", " ---Crab Dampens SQ",
...
tmp_list <- list(sq.sim, sq.wait.sim,
                 ...
                 sdivS.sim,sdivS.wait.sim); names(tmp_list) <- c("Status Quo", " ---Crab Facilitates SQ",
