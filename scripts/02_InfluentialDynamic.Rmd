---
title: "Influential Dynamics: Fishing trade-offs, flexibility"
author: "M Fisher"
date: "2023-07-27"
output: 
  html_document:
...
*finish cleaning up: sdiv section, names saved for lists, names in plots, file names*

# Description 

This RMarkdown explores how alternative configurations of an influential dynamic (differences in early season fishing trade-offs / flexibility) may interact with climate adaptation to affect the outcomes from a Harmful Algal Bloom.

The QNM link associated with this influential dynamic is: 

`Early season crab fishery participation` - - - * `Early season other fishery participation`

In other words, the Status Quo configuration states that early season crab fishery participation has a dampening effect on other fishery participation in the early season. We chose to qualify this effect as uncertain. 

The alternative configurations are:

1. Winter crab has a certain positive effect on winter other fishing  ("Waiting for crab")
2. Winter crab has a certain negative effect on winter crab fishing ("Switching targets")
3. Winter crab has no effect on winter non-crab fishing ("No Effect")

*To allow (2) to play out in the QNM, we also removed the restriction that Total non.crab harvest -> Material wealth & security < Total crab harvest -> Material wealth & security*


Structural changes are implemented for the following strategies:

1. Parametric insurance ("insure")

2. Direct Assistance Disaster Relief ("DR1")

3. Multi-Objective Disaster Relief ("DR2")

4. Supplementary Livelihoods Diversification ("sdiv")


We don't look at the fisheries diversification strategies because these strategies already involve specific structural changes to dynamic we are examining. 
...
Read in the simulation output that used the original assumption / Status Quo configuration of the dynamic. 
```{r}
```
...
```{r sq.sim0, eval=FALSE}
#### Original assumption, should have been read in under "data" section. #### 
#--- model ---#
...
names(out) <- c("Original","Waiting for crab","No Effect","Switching targets")
