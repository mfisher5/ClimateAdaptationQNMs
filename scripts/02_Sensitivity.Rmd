...
Read in simulation output from script 01.
```{r}
simlist <- readRDS(here('data','HAB_sim_out',paste0('HABhi_x_StatusQuo_AllStrat_',nmat/1000,'k.rds')))
```
...
for(v in response.vars[10:12]){
  ...
  write_csv(fit.sq.df, here('data','sensitivity','StatusQuo_x_HABhi_GBMout_2.csv'))
              
}
```

Combine data from batches. 
```{r}
fit.sq.df <- read_csv(here('data','sensitivity','StatusQuo_x_HABhi_GBMout_1.csv')) %>%
  bind_rows(read_csv(here('data','sensitivity','StatusQuo_x_HABhi_GBMout_2.csv')))
```
