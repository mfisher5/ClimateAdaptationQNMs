...
```{r}
# dia model
m.fdiv1 <- model.dia(here::here('data','dia','ExistingFisheriesDiversify_HABhi.dia'))
m.fdiv1 <- enforce.limitation(m.fdiv1)  

# edge constraints
m.fdiv1.constrained <- parse.constraints(c("Crab abundance -> Spr/Sum crab fishery participation < Winter (primary) crab fishery participation -* Winter non.crab fishery participation"),m.fdiv1)
...
fdiv1.sim <- system.simulate(n.sims=nmat, # number of randomly generated matrices to keep
...
# edge constraints
m.fdiv2.constrained <- parse.constraints(c("Crab abundance -> Spr/Sum crab fishery participation < Winter (primary) crab fishery participation -* Winter non.crab fishery participation"),m.fdiv2)
...
fdiv2.sim <- system.simulate(n.sims=nmat, # number of randomly generated matrices to keep
...
ldiv1.sim <- system.simulate(n.sims=nmat, # number of randomly generated matrices to keep
...
ldiv2.sim <- system.simulate(n.sims=nmat, # number of randomly generated matrices to keep
...
Into a list, so it can be called up in later scripts (like 2).
```{r}
out.list <- list(sq.sim,insure.sim,dr1.sim,dr2.sim,fdiv1.sim, fdiv2.sim,sdivS.sim,ldiv1.sim,ldiv2.sim)
saveRDS(out.list,here('data','HAB_sim_out',paste0('HABhi_x_StatusQuo_AllStrat_',nmat/1000,'k.rds')))
```
...
compare.sub <- compare.models.impact.tbl(sim.list=tmp_list,
                                    ...
                                    strong_lower=0.8, weak_lower=0.6, epsilon=1.0E-5, main.axis = 'Y',
                                    variable.key = subvars, variable.order="specific_order",plot=TRUE,plot.perturbed=FALSE, table.palette=table.palette.orange)
```
...
```{r}
compare.sub[[3]] + geom_vline(aes(xintercept=10.5), linewidth=1.5) + 
...
```{r}
tmp_list <- list(ldiv1.sim,ldiv2.sim)
names(tmp_list) <- c("Imposed New Livelihoods", "Invested New Livelihoods")
hab.ldiv.out <- compare.models.impact.tbl(sim.list=tmp_list,
                                          perturb.list=list(c(`HAB`=1),
